...
from . import dataflow_discovery_client
from google.cloud import storage
from ....proto import gcp_resources_pb2
...
def create_python_job(python_module_path: str,
                      ...
                      args: list[str] = None,
                      additional_run_args: list[str] = None):
  ...
  Args:
    ...
    additional_run_args: The list of args that will be directly passed to
      Apachea beam runner.
  ...
  df_client = dataflow_discovery_client.DataflowClient()
  job_id = None
  ...
  for line in sub_process.read_lines():
    ...
    if job_id:
      ...
      job_resource.resource_type = 'DataflowPython'
      job_resource.resource_uri = location
...
def prepare_cmd(project_id, region, python_file_path, args, temp_location,
  ...
  if additional_run_args:
    dataflow_args += additional_run_args
...
def parse_blob_path(path) -> Tuple[str]:
